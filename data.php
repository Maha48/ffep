...
$(document).ready(function(){
    ...
    if (this.value=="time") this.value = "Back";
 ...
 $(document).ready(function () {
    ...
    var searchtxt =$('#sameT-subject').val();
    var searchtxt2 =$('#sameT-times').val();
    if($.trim(searchtxt && searchtxt2)!= ''){
    $.ajax({                
    url:'processind_sst.php',                 
    method:"POST",                
    data:{subject:searchtxt,time:searchtxt2},
...
<h4 id="students-file" style="margin-top:0; margin-bottom:3px;"> Enter The Students Table File</h4>
...
<div id="ssd_scroll">
<div id="SSD_search" style="display:none;">
</div>
<div class="SSD-tables">
sdk,s;cl
<?php
// $a=mysqli_query($connection,"select * from SSD");
// $Ea=mysqli_fetch_array($a);

//  IF(mysqli_num_rows($Ea) >0){
//     $b=mysqli_query($connection,"delete from SSD");

//  }
//  $c=mysqli_query($connection,"insert INTO SSD SELECT DISTINCT e1.*  from Examdata e1 , Examdata e2
...
//  $SSD_sql=mysqli_query($connection,"select * from SSD");
...
<input type="button" id="same_time"  value ="time" >in same time 
</div>
...
<input  type="submit" name="sub-pro" id="sub-pro" value="subjects processing" onclick="myFunction4()">
</form>
<form method="post" action="download.php">
<input type="submit"class="button button5"  value="Download" name="Download" id="Download">
</form>
